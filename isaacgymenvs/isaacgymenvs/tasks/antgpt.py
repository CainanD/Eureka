...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.potentials, self.prev_potentials, self.up_vec, self.heading_vec, self.actions, self.dof_vel, self.vec_sensor_tensor, self.dt, self.up_axis_idx)
...
def compute_reward(
    ...
    # Main kinematic quantities
    vel = root_states[:, 7:10]
    ang_vel = root_states[:, 10:13]

    # Forward progress toward target (Isaac Gym convention)
    progress = potentials - prev_potentials  # positive when getting closer to target

    # Project linear velocity onto desired heading for forward speed
    forward_speed = torch.sum(vel * heading_vec, dim=-1)
    forward_speed_clipped = torch.clamp(forward_speed, min=0.0)

    # Uprightness via alignment of body up with world up axis
    # up_vec is a 3D vector; align with world up basis dimension up_axis_idx
    up_alignment = torch.clamp(up_vec[:, up_axis_idx], 0.0, 1.0)

    # Sideways motion (reduce lateral movement to walk "coldly"/steadily)
    forward_comp = heading_vec * forward_speed.unsqueeze(-1)
    sideways_vec = vel - forward_comp
    sideways_speed = torch.norm(sideways_vec, p=2, dim=-1)

    # Angular motion penalty (reduce body oscillations)
    ang_speed = torch.norm(ang_vel, p=2, dim=-1)

    # Action magnitude and joint velocity penalties (smooth/low-energy movement)
    action_mag = torch.mean(torch.abs(actions), dim=-1)
    joint_vel_mag = torch.mean(torch.abs(dof_vel), dim=-1)

    # Contact force penalty (avoid slamming feet)
    # vec_sensor_tensor is typically [N, S, 6]; use force components (0:3)
    force_components = vec_sensor_tensor[..., 0:3]
    if force_components.dim() == 3:
        # Sum norms across sensors
        contact_force_sum = torch.sum(torch.norm(force_components, dim=-1), dim=-1)
    else:
        # Fallback if shape is [N, 3]
        contact_force_sum = torch.norm(force_components, dim=-1)

    # Temperatures for transformed components (each its own variable)
    prog_temp = 1.0
    vel_temp = 1.0
    upright_temp = 0.5
    side_temp = 0.5
    ...
    # Shaped components (bounded, smooth)
    # Use exp transforms for positive rewards and stability terms
    progress_reward = torch.exp(progress / prog_temp) - 1.0
    forward_reward = torch.exp(forward_speed_clipped / vel_temp) - 1.0
    upright_reward = torch.exp(up_alignment / upright_temp) - 1.0

    side_stability = torch.exp(-sideways_speed / side_temp)        # higher when sideways speed is small
    ang_stability = torch.exp(-ang_speed / ang_temp)               # higher when angular speed is small
    contact_stability = torch.exp(-contact_force_sum / contact_temp)  # higher when contact forces are small
    action_smooth = torch.exp(-action_mag / action_temp)           # higher for small actions
    joint_smooth = torch.exp(-joint_vel_mag / joint_temp)          # higher for small joint speeds
    ...
    # Total reward: encourage forward progress and stable, "cold" gait
    total_reward = (
        w_progress * progress_reward
        + w_forward * forward_reward
        + w_upright * upright_reward
        + w_side * side_stability
        + w_ang * ang_stability
        + w_contact * contact_stability
        + w_action * action_smooth
        + w_joint * joint_smooth
    )
